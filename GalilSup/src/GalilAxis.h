...
public:
  ...
  void initialize_codegen(string &thread_code,
			  string &limit_code,
			  string &dighome_code);

  //Generate code for limits interrupt
  void gen_limitcode(string &limit_code);
  ...
  void gen_EnsureOkToMove(string &tc);
  //Generate axis home routine
  void gen_homecode(string &thread_code);
  //Is the motor in an enabled/go state with current digital IO status
  bool motor_enabled(const char *caller, string &mesg);
  ...
  void executeAutoBrakeOn(int dmov, int csdmov);
  //Execute motor record post function
  void executePost(int dmov);
  //Are all related CSAxes done
  asynStatus allCSAxisDoneMoving(bool *csdmov);
  //Execute auto motor power off
  void executeAutoOff(int dmov, int csdmov);
  //Check velocity and wlp protection
  asynStatus beginCheck(const char *caller, char callaxis, double maxVelocity, bool resetCtrlMessage = true);
  //Check axis limits in requested move direction
  asynStatus checkLimits(const char *caller, char callaxis, double npos);
  //Check axis soft limits in requested move direction
  asynStatus checkSoftLimits(const char *caller, char callaxis, double npos);
  //Check motor record status for this axis
  asynStatus checkMRSettings(const char *caller, char callaxis, bool moveVelocity);
  //Check all axis settings
  asynStatus checkAllSettings(const char *caller, char callaxis, double position, double maxVelocity);
  //Begin motor motion
  asynStatus beginMotion(const char *caller, double position = 0.0, bool relative = false, bool checkpos = false, bool move = true);
  //Jog after home
  asynStatus jogAfterHome(void);
  ...
  asynStatus moveThruMotorRecord(double position, bool signalCaller = false);
  //Driver internal version of axis stop, prevents backlash, retries till dmov
  asynStatus stopInternal(double acceleration);
  //Driver stops motor record.  Prevents backlash, retries.
...
private:
  ...
  char csaxesList_[MAX_GALIL_AXES+1];	//CSAxis list that include this axis
